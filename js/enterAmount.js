...
function verify(amount) {
    ...
    if (temp_transaction.amount > amountAvailable.amount) {
        ...
        if ((temp_transaction.amount % 50) != 0) {
        ...
        temp_transaction.fifty_rs_note = Math.trunc(temp_transaction.amount / 50);
        temp_transaction.amount -= temp_transaction.fifty_rs_note * 50;
    ...
    amountAvailable.fifty_rs_note -= temp_transaction.fifty_rs_note;
...
function checkNotes(temp_transaction) {
    ...
    if (amountAvailable.hundred_rs_note < temp_transaction.hundred_rs_note) {
        temp_transaction.fifty_rs_note += 2 * (temp_transaction.hundred_rs_note - amountAvailable.hundred_rs_note);
    }

    if (amountAvailable.fifty_rs_note < temp_transaction.fifty_rs_note) return false;
    return true;
